(async function () {
  ...
  fs.writeFileSync(path.resolve(__dirname, '../data/tickets.json'), JSON.stringify(diff.concat(tickets)), 'utf8')
  npm.search.issuesAndPullRequests({ q: queryNPM }).then(response => {
    const issues = response.data.items
    issues = issues.filter(i => i.url)
    console.log(issues)
    // github.search.issuesAndPullRequests({ q: queryGitHub }).then(response => {
    //   let existing = response.data.items.map(i => i.title)
    //   issues.filter(i => !existing[i.title]).map(i => {
    //     let { title, url, labels } = i
